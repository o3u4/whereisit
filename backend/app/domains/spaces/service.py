...
def to_item(conn: sqlite3.Connection, user_id: int, space_id: int, *, qty: int = 1) -> dict:
    """Turn a leaf space into an item of the same name, placed where the space
    was (its parent). Lots sitting inside are promoted to the parent first.
    Refuses when sub-spaces exist or the space is a root (no parent to hold it)."""
    node = require(conn, user_id, space_id)
    if _children_of(conn, user_id, space_id):
        raise BadRequest("还有子空间，先移走或删除它们")
    parent = node["parent_id"]
    if parent is None:
        raise BadRequest("顶层空间不能转为物品")
    moved = conn.execute(
        "UPDATE item_lots SET space_id = ?, updated_at = datetime('now') "
        "WHERE space_id = ? AND owner_id = ?",
        (parent, space_id, user_id),
    ).rowcount
    from app.domains.items import service as items_service  # lazy: items imports spaces
